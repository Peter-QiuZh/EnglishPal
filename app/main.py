...
def userpage(username):
    ...
    if datetime.now().strftime('%Y%m%d') > user_expiry_date:
        return '<p>账号 %s 过期。</p><p>为了提高服务质量，English Pal 收取会员费用， 每天0元。</p> <p>请决定你要试用的时间长度，扫描下面支付宝二维码支付。 支付时请注明<i>English Pal Membership Fee</i>。 我们会于12小时内激活账号。</p><p><img src="static/donate-the-author-hidden.jpg" width="120px" alt="支付宝二维码" /></p><p>如果有问题，请加开发者微信 torontohui。</p> <p><a href="/logout">退出</a></p>' % (username)
    ...
    if request.method == 'POST':  # when we submit a form
        ...
        page += '''
                ...
                '''
        page += '<div class="container-fluid">'
        page += '<p><b>English Pal for <font id="username" color="red">%s</font></b> <a class="btn btn-secondary" href="/logout" role="button">退出</a></p>' % (username)
        ...
        if len(d) > 0:
            ...
            sorted_lst = sort_in_descending_order(lst2)
            for x in sorted_lst:
                word = x[0]
                freq = x[1]
...
def login():
    if request.method == 'GET':
        if not session.get('logged_in'):
            ...
            return '你已登录 <a href="/%s">%s</a>。 退出点击<a href="/logout">这里</a>。' % (session['username'], session['username'])
    elif request.method == 'POST':
